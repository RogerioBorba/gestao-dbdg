...
<div class="flex mt-1 relative text-gray-700">
    <p class="flex-grow text-grey-darkest hover:bg-red truncate text-left text-xs" title="{layer.title()}">{layer.title()}</p>
    <button class="focus:outline-none bg-grey-light hover:bg-grey text-grey-darkest font-bold py-1 px-1 rounded inline-flex items-center hover:bg-gray-200" on:click|stopPropagation={btnClearClicked} title="Remover camada">
